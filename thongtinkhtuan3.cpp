...
{
	if(this->ns.nam < a.ns.nam)
	{
		return true;
		if(this->ns.thang > a.ns.thang)
		{
			return true;
			if(this->ns.ngay > a.ns.ngay)
				return true;
		}
	}
	return false;	
...
{
	cout<<"\n\t\t\tDANH SACH DA DUOC SAP XEP THEO TUOI GIAM DAN\n";
	for(int i=0;i<n;i++)
		for(int j=i+1;j<n;j++)
		if(a[j]> a[i])
		{
...
void KH ::HK_HN(KH a[],int n)
{
	cout<<"\n\t\t\tXUAT CAC KHACH HANG CO HO KHAU LA HA NOI\n";
	int check =0;
	for(int i=0;i<n;i++)
	{
		string temp=a[i].ho_khau;
		if(temp=="Ha Noi")
		{
			++check;
			if(check == 1)
	...
	if(check ==0)
		cout<<"\n\t\t\tKHONG CO KHACH HANG NAO CO HO KHAU LA HA NOI\n";	
}
int main()
{
	int n;
